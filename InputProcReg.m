function [NGP, Nodes, Edges, Loops, BConds, NoDiv, StructDef] = InputProcReg
...
% * from STRUCTREGBC2: edgesDirichlet, edgesNeumann, dataDir, dataNeu;
...
Edges=struct('nini',edges_nodes(:,1),'nfin',edges_nodes(:,2),...
    ...
    zeros(length(edges_nodes(:,1)),2));  % !!! DIFERENCIAÇÃO NAS DIREÇÕES NORMAL E TANGENCIAL
Edges.type(:) = 'D';           % all edges are predefined as Dirichlet 
Edges.order(:,:) = NaN;          % all degrees are predefined as NaN %ALTERAÇÃO PARA ATRIBUIR NAN A NOVA COLUNA CRIADA
...
% boundaries.
% A refinement order different from NaN is only set to the Dirichlet
% boundaries which have non-NaN boundary conditions defined on them. This
% means that a NaN order means that no traction approximation is made in
% that direction.
for i = 1:length(edges_nodes(:,1))
    if strcmpi(Edges.type(i),'D') % selects the Dirichlet boundaries
        if(~Edges.lright(i))      % exterior Dirichlet boundaries
            if any(~isnan(BConds.Dirichlet{i,1}))
                Edges.order(i,1) = EdgesOrder;
            end
            if any(~isnan(BConds.Dirichlet{i,2}))
                Edges.order(i,2) = EdgesOrder;
            end
        else       % interior boundaries
            Edges.order(i,:) = EdgesOrder;
        end
    end
end
% ... or overwrite orders manually, if you need to have different orders
% Edges.order(13,1) = 3;       <------- Overwrite for the normal direction
% Edges.order(17,2) = 5;       <------- Overwrite for the tangential direction
...
for ii=1:length(Edges.type)
    if Edges.type(ii)=='D' && any(~isnan(cell2mat(BConds.Neumann(ii,1)))) ...
        ...
        error('local:consistencyChk',...
            ...
            ii);
    elseif(Edges.type(ii)=='D' && all(isnan(Edges.order(ii,:))))
        % if Dirichlet boundary and all orders are NaN
        error('local:consistencyChk',...
