function LHS = B2(Edges, Loops, LHS, abscissa, weight)
        ...
        if LocEdge.lleft
            ...
            for jj = 1:2 
                ...
                if LocEdge.dim(jj)
                    ...
                    % the jj direction.
                    B2i = sign*B2_Matrix_i(LocEdge, LocLoop, jj, abscissa, weight);
...
function B2i = B2_Matrix_i(LocEdge, LocLoop, jj, abscissa, weight)
...
B2i = zeros(LocLoop.dim,LocEdge.dim(jj));
...
B2i3D = U.*Z;
w3D(1,1,:) = weight;
B2i = L/2 * sum(bsxfun(@times,B2i3D,w3D),3); % computes the integral
