function Abar = B2bar(Edges, Loops, Abar, abscissa, weight, Dim, ii, a)
% B2 sweeps through the edges and calls the functions that generate the
% B2 block of the boundary matrix in the LHS.
%
% B2 is called by MAIN***. It receives as input data the Edges and Loops
...
% and weights. It returns to MAIN*** the LHS matrix with the B2 blocks of
...
%
%
% B2 computes the internal product between the following bases, expressed
% in a polar (r,th) referential:
% * the displacement basis U2, generated by even harmonic potentials,
%        U2 = | Ur | = r^n * | cos[(n+1)*th] |
%             | Ut |         |-sin[(n+1)*th] |
% * the boundary traction basis Z, defined by Chebyshev polynomials,
%        Z  = cos(m*ArcCos(abscissa))
% where n and m are the line and column of the current term in matrix B2.
%
...
if strcmpi(Edges.type(ii),'D')
    ...
    if LocEdge.lleft
        ...
        LocLoop = struct('id',id,'edges',Loops.edges(id,:),... 
            ...
            'insert',Loops.insert(id,2),'dim',Loops.dim(id,2));
        ...
        for jj = a
            ...
            if LocEdge.dim(jj)
                ...
                % the jj direction.
                B2bari = sign*B2_Matrix_i(LocEdge, LocLoop, jj, abscissa, weight);
                % Inserting the block in the global LHS matrix. The 
                % insertion is made at line LocLoop.insert & column 
                % LocEdge.insert(jj).
                Abar(LocLoop.insert:LocLoop.insert+LocLoop.dim-1,Dim+1) = -B2bari;
                % Inserting the conjugate transposed in the global LHS matrix
                Abar(Dim+1,LocLoop.insert:LocLoop.insert+LocLoop.dim-1) = -B2bari';
            end
        end 
    else                       
...
function B2bari = B2_Matrix_i(LocEdge, LocLoop, jj, abscissa, weight)
% B2_MATRIX_I local function computes the B2 boundary block for the LocEdge
% edge and LocLoop element, in the jj direction. The side is mapped to a 
% [-1,1] interval to perform the integration.

%% Initialization 
% Initialization of the B2 block
B2bari = zeros(LocLoop.dim,LocEdge.dim(jj));
...
B2bari3D = U.*Z;
w3D(1,1,:) = weight;
B2bari = L/2 * sum(bsxfun(@times,B2bari3D,w3D),3); % computes the integral
