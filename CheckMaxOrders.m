function [ErrorEnergyNorm,Stop,MaxOrder]=CheckMaxOrders(Loops,Edges,ErrorEnergyNorm)
...
% For the boundaries
for i=1:length(Edges.order)
    for j=1:size(Edges.order,2)
      if (Edges.order(i,j) >= MaxOrder)
          ErrorEnergyNorm=0.0;
          Stop=1;
      end
end
