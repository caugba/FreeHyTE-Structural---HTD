function Abar = B1bar(Edges, Loops, Abar, abscissa, weight, Dim, ii,a)
% B1 sweeps through the edges and calls the functions that generate the
% B1 block of the boundary matrix in the LHS.
%
% B1 is called by MAIN***. It receives as input data the Edges and Loops
...
% and weights. It returns to MAIN*** the LHS matrix with the B1 blocks of
...
%
%
% B1 computes the internal product between the following bases, expressed
...
% where n and m are the line and column of the current term in matrix B1.
%
    ...
    if LocEdge.lleft
        ...
        for jj = a 
            ...
            if LocEdge.dim(jj)    
                ...
                % the jj direction.
                B1bari = sign*B1_Matrix_i(LocEdge, LocLoop, jj, abscissa, weight);
                % Inserting the block in the global LHS matrix. The 
                % insertion is made at line LocLoop.insert & column 
                % LocEdge.insert(jj).
                Abar(LocLoop.insert:LocLoop.insert+LocLoop.dim-1,Dim+1) = -B1bari;
                % Inserting the conjugate transposed in the global LHS matrix
                Abar(Dim+1,LocLoop.insert:LocLoop.insert+LocLoop.dim-1) = -B1bari';
            end
        end 
...
function B1bari = B1_Matrix_i(LocEdge, LocLoop, jj, abscissa, weight)
% B1_MATRIX_I local function computes the B1 boundary block for the LocEdge
% edge and LocLoop element, in the jj direction. The side is mapped to a 
% [-1,1] interval to perform the integration.

%% Initialization 
% Initialization of the B1 block
B1bari = zeros(LocLoop.dim,LocEdge.dim(jj));
...
B1i3D = U.*Z;
w3D(1,1,:) = weight;
B1bari = L/2 * sum(bsxfun(@times,B1i3D,w3D),3); % computes the integral
