...
% COMPUTEFIELDSREG computes, stores and plots the final displacement and
% stress fields in the structure.
%
%
% COMPUTEFIELDSREG is called by MAINREG. It receives structures Edges and 
% Loops, the solution vector X (calculated in MAINREG), the node position 
% list Node and the number of divisions for plotting in the Cartesian
% directions, NoDiv (by default defined as equal to the number of
% Gauss-Legendre integration points, see INPUTPROCREG). 
% Besides the input arguments, COMPUTEFIELDSREG loads from STRUCTDEF.mat
...
% are returned to MAINREG, for some eventual posterior use;
% * storing the displacement and stress estimates, calculated in the 
...
% of reference [2]. The output options are explained in detail in 
% Section 7.2 of reference [2].
...
for ii=1:length(Loops.area)
    ...
    Sr = sum(bsxfun(@times,S1r,X1),3) + sum(bsxfun(@times,S2r,X2),3) + ...
        sum(bsxfun(@times,S3r(:,:,2:end),X3),3) + ...
        sum(bsxfun(@times,S4r,X4),3); 
    
    St = sum(bsxfun(@times,S1t,X1),3) + sum(bsxfun(@times,S2t,X2),3) + ...
        sum(bsxfun(@times,S3t(:,:,2:end),X3),3) + ...
        sum(bsxfun(@times,S4t,X4),3); 
    
    Srt = sum(bsxfun(@times,S1rt,X1),3) + sum(bsxfun(@times,S2rt,X2),3) + ...
        sum(bsxfun(@times,S3rt(:,:,2:end),X3),3) + ...
        sum(bsxfun(@times,S4rt,X4),3); 
...
% last one alone.
subplot(4,2,1); caxis(caxis); colorbar;
subplot(4,2,2); caxis(caxis); colorbar;
subplot(4,2,3); caxis(caxis); colorbar;
subplot(4,2,4); caxis(caxis); colorbar;
subplot(4,2,5); caxis(caxis); colorbar;
...
% vertices of the elements because of the shift in the limits of the
% plotting interval (see the definition of the 'pts' variable above).

% The displacements are scaled such that the maximum displacement in the 
% structure is represented as roughly 10% of the diagonal of the structure
% Getting the vertices of the structure.
xmax = (max(Nodes(Loops.nodes(:),1)));
xmin = (min(Nodes(Loops.nodes(:),1)));
ymax = (max(Nodes(Loops.nodes(:),2)));
ymin = (min(Nodes(Loops.nodes(:),2)));
% Computing the diagonal of the structure
