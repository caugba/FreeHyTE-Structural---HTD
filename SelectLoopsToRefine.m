...
% of a boundary basis causes the instability of the system, 
% new modes are added to the finite elements adjacent to that boundary
% until the system get stable.
%
% Further details on the refinement selection process can be found in
% reference [2] (Section 6.3.2), and reference [4] (Sections 4.6.1 and 5.4.5).
...
for i=1:length(Loops.area)
    ...
    % boundaries.
    List.BetaIt(iteration,i)= sum(Loops.dim(i,:)) - sum(sum(Edges.dim(Loops.edges(i,:),:)),2);
...
for i=1:length(Loops.area)
    ...
    if Loops.order(i) <= max(max(Edges.order(Loops.edges(i,:),:)))
        List.LoopsToRefine = cat(2,List.LoopsToRefine,i);
    end
