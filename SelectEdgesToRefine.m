...
% This happens when the free vector of system (4.2) in reference [4] is
...
% reference [4] (Sections 4.6.1 and 5.4.5).
...
List.EdgesToRefine(1,1) = List.Edge(1,1);
List.EdgesToRefine(1,2) = List.Edge(1,2);
...
for i=2:length(List.Edge)
    if List.Edge(i,SelectionCriterion) >= ...
        ...
        List.EdgesToRefine(listposition,1)= List.Edge(i,1);
        List.EdgesToRefine(listposition,2)= List.Edge(i,2);
...
for index=1:size(List.Edge,1)
    if List.Edge(index,2)<thresh
        ...
        List.EdgesToRefine(listposition,1)= List.Edge(index,1);
        List.EdgesToRefine(listposition,2)= List.Edge(index,2);
        listposition = listposition + 1;
    end
end
