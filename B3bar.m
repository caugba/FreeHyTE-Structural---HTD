function Abar = B3bar(Edges, Loops, Abar, abscissa, weight, Dim, ii, a)
    ...
    if LocEdge.lleft
        ...
        for jj = a
            ...
            if LocEdge.dim(jj)
                ...
                % the jj direction.
                B3bari = sign*B3_Matrix_i(LocEdge, LocLoop, jj, abscissa, weight);
                % Inserting the block in the global LHS matrix. The 
                % insertion is made at line LocLoop.insert & column 
                % LocEdge.insert(jj).
                Abar(LocLoop.insert:LocLoop.insert+LocLoop.dim-1,Dim+1) = -B3bari;
                % Inserting the conjugate transposed in the global LHS matrix
                Abar(Dim+1,LocLoop.insert:LocLoop.insert+LocLoop.dim-1) = -B3bari';
            end
        end
...
function B3bari = B3_Matrix_i(LocEdge, LocLoop, jj, abscissa, weight)
...
B3bari = zeros(LocLoop.dim,LocEdge.dim(jj));
...
B3bari3D = U.*Z;
w3D(1,1,:) = weight;
B3bari = L/2 * sum(bsxfun(@times,B3bari3D,w3D),3); % computes the integral
