function Ubar = Ubari(Edges, BConds, abscissa, weight, ii, a)
...
% * the boundary displacement function is defined in the GUbar by its values 
...
if (strcmpi(Edges.type(ii),'D') && Edges.lright(ii) == 0 )
    ...
    for jj = a
        ...
        if LocEdge.dim(jj)
            ...
            Ubar = U_Vector_i(LocEdge, BConds, jj, abscissa, weight, Ubar);
...
function Ubar = U_Vector_i(LocEdge, BConds, jj, abscissa, weight, Ubar)
...
Ubar2D = bsxfun(@times, Z, q);

% Performing the side integration 
Ubar = L/2 * sum(bsxfun(@times,Ubar2D,weight),1); 
Ubar = Ubar.';
