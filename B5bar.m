function Abar = B5bar(Edges, Loops, Abar, abscissa, weight, Dim, ii, a)
    ...
    if LocEdge.lleft
        ...
        for jj = a 
            ...
            if LocEdge.dim(jj)
                ...
                B1rbari = sign*B1r_Matrix_i(LocEdge, LocLoop, jj, abscissa, weight);
                ...
                Abar(LocLoop.insert, Dim+1) = -B1rbari;
                % Inserting the conjugate transposed in the global LHS matrix
                Abar(Dim+1, LocLoop.insert) = -B1rbari';

                % Launching the function that computes the boundary
                % block for the current edge and the second rigid body
                % mode of its left element, in the jj direction.
                B2rbari = sign*B2r_Matrix_i(LocEdge, LocLoop, jj, abscissa, weight);
                % Inserting the block in the global LHS matrix. The
                % insertion is made at line LocLoop.insert+1 (the 2nd 
                % rigid body mode line)& column LocEdge.insert(jj).
                Abar(LocLoop.insert +1 ,Dim+1) = -B2rbari;
                % Inserting the conjugate transposed in the global LHS matrix
                Abar(Dim+1,LocLoop.insert+1 ) = -B2rbari';

                % Launching the function that computes the boundary
                % block for the current edge and the third rigid body
                % mode of its left element, in the jj direction.
                B3rbari = sign*B3r_Matrix_i(LocEdge, LocLoop, jj, abscissa, weight);
                % Inserting the block in the global LHS matrix. The
                % insertion is made at line LocLoop.insert+2 (the 3rd 
                % rigid body mode line)& column LocEdge.insert(jj).
                Abar(LocLoop.insert+2,Dim+1) = -B3rbari;
                % Inserting the conjugate transposed in the global LHS matrix
                Abar(Dim+1,LocLoop.insert+2) = -B3rbari';
            end
        end 
...
function B1rbari = B1r_Matrix_i(LocEdge, LocLoop, jj, abscissa, weight)
...
B1rbari = zeros(LocLoop.dim,LocEdge.dim(jj));
...
B1rbari3D = U.*Z;
w3D(1,1,:) = weight;
B1rbari = L/2 * sum(bsxfun(@times,B1rbari3D,w3D),3); % computes the integral

end



function B2rbari = B2r_Matrix_i(LocEdge, LocLoop, jj, abscissa, weight)
...
B2rbari = zeros(LocLoop.dim,LocEdge.dim(jj));
...
B2rbari3D = U.*Z;
w3D(1,1,:) = weight;
B2rbari = L/2 * sum(bsxfun(@times,B2rbari3D,w3D),3); % computes the integral

end



function B3rbari = B3r_Matrix_i(LocEdge, LocLoop, jj, abscissa, weight)
...
B3rbari = zeros(LocLoop.dim,LocEdge.dim(jj));
...
B3rbari3D = U.*Z;
w3D(1,1,:) = weight;
B3rbari = L/2 * sum(bsxfun(@times,B3rbari3D,w3D),3); % computes the integral
