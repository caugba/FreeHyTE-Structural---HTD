function varargout = AdaptiveTri(varargin)
% ADAPTIVETRI MATLAB code for AdaptiveTri.fig
%      ADAPTIVETRI, by itself, creates a new ADAPTIVETRI or raises the existing
%      singleton*.
%
%      H = ADAPTIVETRI returns the handle to a new ADAPTIVETRI or the handle to
%      the existing singleton*.
%
%      ADAPTIVETRI('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in ADAPTIVETRI.M with the given input arguments.
%
%      ADAPTIVETRI('Property','Value',...) creates a new ADAPTIVETRI or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before AdaptiveTri_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to AdaptiveTri_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help AdaptiveTri

% Last Modified by GUIDE v2.5 07-Oct-2016 18:12:40
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @AdaptiveTri_OpeningFcn, ...
                   'gui_OutputFcn',  @AdaptiveTri_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% UIWAIT makes AdaptiveTri wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = AdaptiveTri_OutputFcn(hObject, eventdata, handles) 
...
% --- Executes just before AdaptiveTri is made visible.
function AdaptiveTri_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to AdaptiveTri (see VARARGIN)

% Choose default command line output for AdaptiveTri
