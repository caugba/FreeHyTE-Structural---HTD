...
if  CndNo<eps %2.2204e-16
    % checking if it's inferior to the machine precision
...
if(RunOption==1)
    ...
    [~, ~, ~, ~, ~] = ComputeFieldsReg(NoDiv, Nodes, Edges, Loops,X);
...
% Saving the scaled coefficients matrix from the initial run to avoid its
% recalculation
ScLHS0=ScLHS; 

% Storing the initial refinements. Since no system instability was identified 
% in the first run, the solving system is assumed stable. Edges.stable and 
...
% in Sections 5.3.1 and 6.1 of reference [3], respectively.
...
% SelectionCriterion=  %1 bnd ; 2 energy
% StoppingCriterion=  %1 energy ; 2 bnd
...
% and in Chapters 4 of reference [4].
%
...
%     end 
  ...
  %% Rebuilding and solving the finite element system
    ...
    while  Outliers && ~RevertAndExit % runs while outliers are detected
        ...
        MaxOrder = 20;
        [ErrorNorm,Stop,MaxOrder]=CheckMaxOrders(Loops,Edges,ErrorNorm);       
    ...
    % - Stop = 0 (Máxima ordem atingida) & RevertAndExit = 2(MaxOutlierIter iterations atingida): max refinement reached and max outlier iterations reached - revert & quit
